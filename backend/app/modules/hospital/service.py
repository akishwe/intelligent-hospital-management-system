...
class HospitalService:
    ...
    def get_hospitals(db: Session, skip: int = 0, limit: int = 100) -> List[Hospital]:
        return db.query(Hospital).filter(Hospital.is_deleted == False).offset(skip).limit(limit).all()

    @staticmethod
    def update_hospital(db: Session, hospital_id: int, hospital_data: HospitalUpdate) -> Hospital:
